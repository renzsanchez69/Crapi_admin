...
class MY_Model extends Base_Model {
	...
	public function paginate($page = 1, $where = array(), $limit = NULL, $joins = array(), $select = array(), $sort = null)
	{
		...
		if (!empty($joins)) {
			foreach ($joins as $join) {
				...
				$this->db->join($join['table'].' '.$join['alias'], $conditions, $join['type']);
			}
	...
	public function fetch_many($where = array(), $joins = array(), $select = array())
	{
		// avoid overrided by $this->limit
		$this->limit = NULL;
		...
		return array('data' => $results, 'counts' => $count_total);
	}
}