...
class Form {
	...
	public function field_number($name, $value = NULL, $extra = array())
	{
		$data = array('type' => 'number', 'id' => $name, 'name' => $name);
		$value = ($value===NULL) ? $this->get_field_value($name) : $value;
		return form_input($data, $value, $extra);
	}
	 ...
	 */
	public function bs3_datetimepicker($label, $name, $value = NULL, $extra = array())
	{
		$extra['class'] = 'form-control datetimepicker';
		return '<div class="form-group">'.form_label($label, $name).$this->field_text($name, $value, $extra).'</div>';
	}
	public function bs3_timepicker($label, $name, $value = NULL, $extra = array())
	{
	...
	public function bs3_radio($label, $name, $options = array(), $selected = NULL, $required = false){
		$rdo_options = '';
		foreach ($options as $rdo_key => $rdo_value) {
			$checked = isset($selected) && $rdo_key == $selected ? 'checked' : '';
			$required = is_bool($required) && $required == true ? 'required' : '';
			$rdo_options .= '<label class="radio-inline">';
			$rdo_options .= '<input type="radio" name="'.$name.'"value="'.$rdo_key.'" '.$checked.' '.$required;
			$rdo_options .= '>'.ucwords($rdo_value).'</label>';
		}
		return '<div class="form-group">'.form_label($label, $name).'</br>'.$rdo_options.'</div>';
	}
	...
	public function bs3_hidden($label, $name, $value = NULL, $extra = array())
	{
		$extra['class'] = 'form-control';
		return '<div class="form-group">'.form_label($label, $name).$this->field_hidden($name, $value, $extra).'</div>';
	}

	public function bs3_number($label, $name, $value = NULL, $extra = array())
	{
		$extra['class'] = 'form-control';
		return '<div class="form-group">'.form_label($label, $name).$this->field_number($name, $value, $extra).'</div>';
	}


	public function bs3_submit($label = 'Submit', $style = 'primary', $extra = array())
	{
		!empty($extra['class']) ? $extra['class'] .= ' btn btn-'.$style : $extra['class'] = 'btn btn-'.$style;
